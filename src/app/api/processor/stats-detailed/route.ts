...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Рейтинг процессоров (топ по месяцу)
    const topProcessors = await prisma.processor_deposits.groupBy({
      by: ['processorId'],
      where: {
    ...
    // Получаем информацию о процессорах для рейтинга
    const processorIds = topProcessors.map(p => p.processorId);
    const processors = await prisma.users.findMany({
      where: { id: { in: processorIds } },
      select: { id: true, name: true, email: true }
    });

    const leaderboard = topProcessors.map((stats, index) => {
      const processor = processors.find(p => p.id === stats.processorId);
      const name = processor?.name || processor?.email || 'Unknown';
    ...
    const stats = {
      ...
      settings: {
        ...
        baseBonusRate: bonusSettings?.baseBonusRate || 5.0,
