...
export default function AdminUserSection() {
  ...
  if (!user) {
    // Если не авторизован — показываем призыв войти, чтобы блок всегда был на месте
    return (
      <div className="border-t border-black/5 dark:border-white/10 p-3 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-sm">
        <div className="mb-2 text-sm text-[#171717]/70 dark:text-[#ededed]/80">Вы не вошли в систему</div>
        <div className="flex gap-2">
          <Link href="/login" className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium text-white bg-gray-900 hover:bg-gray-800 dark:bg-white/10 dark:hover:bg-white/20 rounded-md transition-colors">Войти</Link>
          <Link href="/register" className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium text-[#171717]/70 dark:text-[#ededed]/80 bg-[#171717]/5 dark:bg-[#ededed]/10 hover:bg-[#171717]/10 dark:hover:bg-[#ededed]/20 rounded-md transition-colors">Регистрация</Link>
        </div>
      </div>
    );
  }
